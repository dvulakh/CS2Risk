...
public class Territory {
	...
	public void occupy(Player p, int t){
		...
		if(BoardState.BOARD != null && BoardState.BOARD.getGraphics() != null){
			if(BoardState.BOARD.getAttackMenu() == null && BoardState.BOARD.getMoveMenu() == null)
	...
	public void conquestPaint(Graphics g){
		floodTerr(g);
		BoardState.paintAdjacent(g, this);
		for(Territory t: BoardState.territories)
			if(t.isAdjacent(this))
				t.paint(g);
		paint(g);
	}
	public void paint(Graphics g){
		if(BoardState.BOARD.showCount){
			...
			if(col != ATTACK_COL || (!BoardState.BOARD.flood && (BoardState.phase == BoardState.ATTACK || BoardState.phase == BoardState.FORTIFY)))
				g.setColor(col);
			g.fillOval(l[0], l[1], 2 * RAD(), 2 * RAD());
			if(occupation != null){
				g.setColor(!BoardState.BOARD.flood && col != ATTACK_COL ? occupation.getColor() : GameBoard.FONT);
				if(BoardState.BOARD.flood && (BoardState.phase == BoardState.ATTACK || BoardState.phase == BoardState.FORTIFY) && col == ATTACK_COL)
