...
public class GameBoard extends JFrame implements MouseListener, MouseMotionListener, WindowListener, ActionListener {
	...
	public MoveMenu getMoveMenu(){return move;}
	public Console getConsole(){return sideConsole;}
	public JButton[] getBottomButtons(){return bottomButtons;}
	...
	public void mouseMoved(MouseEvent arg0) {
		...
		for(Territory t: BoardState.territories)
			if(Math.sqrt(Math.pow(getMousePosition().getX() - t.getCLocX(), 2) + Math.pow(getMousePosition().getY() - t.getCLocY(), 2)) <= Territory.RAD())
				tMouse = t;
		if(tMouse != moused && attack == null && move == null){
			if(moused != null)
	...
	public void mouseClicked(MouseEvent arg0) {
		...
		if(BoardState.phase == BoardState.FORTIFY && move == null){
			clicked = moused;
			BoardState.pTurn().maneuvers();
			return;
		}
	}
	...
	public void actionPerformed(ActionEvent e) {
		...
		//Retreat
		if(attack != null && e.getSource() == attack.buttons[0] && move == null){
		...
		if(attack != null && e.getSource() == attack.buttons[4])
			while(attack.attacker.getTroops() > 1 && attack.attacker.getOccupation() != attack.defender.getOccupation())
				BoardState.attack(attack.attacker, attack.defender, Math.min(attack.attacker.getTroops() - 1, 3));
